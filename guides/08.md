...
```mermaid
flowchart LR
    A["① 小さく作る"] --> B["② localhost で確認"] --> C["③ git commit"] --> D["④ git push"] --> E["⑤ 公開サイトで確認"]
    E --> A
```
...
> - `#[Fillable]` への追加忘れは定番のつまずきポイントです。保存されないときはここを疑うよう伝えてください
...
- チェックボックスは値が送られないことがあるので `$request->boolean('is_published')` を使うと安全です
...
```mermaid
flowchart LR
    Guest["👤 訪問者"] -->|"読める"| Read["📰 一覧・記事ページ"]
    Guest -->|"✋ 入れない"| Write["✏️ 作成・編集・削除"]
    Owner["🔑 ログイン中の自分"] -->|"読める"| Read
    Owner -->|"書ける"| Write
```
...
Laravel には、**認証機能が最初から入ったテンプレート**が用意されています。
...
そのあと、ログイン画面のビューを自分で用意し、`routes/web.php` で
**書き込み系のルートだけ**を保護します。
...
> ⚠️ `/articles/create` は `/articles/{article}` **より上**に書く必要があります（04 参照）。
> グループを分けるときも、この順番に気をつけてください。
...
> **メンター視点**
> - 既存プロジェクトへの Fortify 追加は難易度が高めです。**まずスターターキットで動くものを見せる**のが近道です
...
> ⚠️ **公開サイトでの注意**
> Laravel Cloud はデプロイのたびにファイルが消えるため、
> アップロードした画像は **オブジェクトストレージ（S3互換）** に保存する必要があります。
...
- 中間テーブルの名前は **アルファベット順・単数形**（`article_tag`）が規約です
...
- `league/commonmark` の `html_input` を `strip` に設定して、危険なタグを取り除きましょう
...
```
❌ いきなり完璧なタグ機能を作ろうとする
✅ まず tags テーブルだけ作って、php artisan migrate してみる
```
...
```mermaid
flowchart LR
    E["❌ エラーが出た"] --> R["📖 エラーを読む"] --> S["🔍 検索する"] --> F["✅ 解決！"] --> L["💪 一番の学び"]
```
...
あなたはもう、**動くWebアプリを作って、インターネットに公開した**経験があります。
