...
> ⚠️ このヒントのコードには、月の絞り込みが入っていません
> `index()` の中身をこのまま置き換えると、月別アーカイブのセレクトボックスは
> 画面に残ったまま、選んでも何も変わらなくなります（壊れたわけではありません）。
> どちらも動かしたい場合は、`$query` に月の条件も足します。
>
> ```php
> $query->whereYear('published_on', substr($month, 0, 4))
>       ->whereMonth('published_on', substr($month, 5, 2));
> ```
>
> このとき、検索フォームと月のフォームは別々なので、片方を送るともう片方が消えます。
> 両方を保つには、それぞれのフォームに `<input type="hidden">` で相手の値を入れておきます。

> メンター視点
> - `where` の中に関数を入れる書き方は難しいので、まずタイトルだけの検索から始めさせると挫折しにくいです
> - 月別アーカイブと検索を両立させるのは少し難しい課題です。片方ずつでもOKと伝えてください
> - 「月別が効かなくなった」という質問が出たら、壊れたのではなく条件を消したためだと説明してください
...
> 💡 ページ送りのボタンが素っ気ない見た目になったら
> ボタンのスタイルは、Laravel があらかじめ用意しているビューの中にあります。
> `npm run dev` を起動していれば、そのまま反映されます。
> `npm run build` で進めている場合は、もう一度実行してください。

> 💡 `Showing 11 to 17 of 17 results` と英語で出ます
> 日本語にしたい場合は、以下でビューを取り出して編集できます（任意）。
> ```bash
> php artisan vendor:publish --tag=laravel-pagination
> ```
> `resources/views/vendor/pagination/tailwind.blade.php` が編集できるようになります。
...
`POST /logout` は Fortify が用意してくれていますが、押す場所は自分で作ります。
`layouts/app.blade.php` のヘッダーを、ログイン状態で出し分けます。
...
@auth
    <div class="flex items-center gap-2">
        ...
        <form method="POST" action="/logout">
            @csrf
            <button type="submit" class="rounded-lg border border-gray-300 bg-white px-4 py-2 shadow-sm hover:bg-gray-50">
                ログアウト
            </button>
        </form>
    </div>
@else
    <a href="/login" class="rounded-lg bg-blue-600 px-4 py-2 text-white shadow-sm hover:bg-blue-700">
        ログイン
    </a>
@endauth
```
...
| 書いたコード | 意味 |
|---|---|
| `@auth ... @else ... @endauth` | ログイン中と未ログインで表示を変える |
| `<form method="POST">` | ログアウトはリンクでは押せません（後述） |
| `@csrf` | 05 で使ったものと同じ。なりすまし送信を防ぐ合言葉 |

> ⚠️ ログアウトはリンク（`<a href="/logout">`）では動きません
> `/logout` は POST でしか受け付けないため、リンクにすると
> `405 Method Not Allowed` になります。上の例のようにフォームとボタンで作ってください。

うまくいくと、こうなります。

| 状態 | `/`・記事ページ | `/articles/create` | ヘッダー |
|---|---|---|---|
| 未ログイン | 表示される | `/login` に飛ばされる | ログイン |
| ログイン中 | 表示される | 表示される | + 新しい記事 / ログアウト |
...
> - ④ を飛ばすと、ログインしたあと自分でログアウトできなくなります。
>   その場合はブラウザの Cookie を消せば戻せますが、先に作っておくほうが楽です
