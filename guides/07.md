...
flowchart LR
    ...
    GH -->|自動で反映| R["☁️ Render"]
    R --> URL["🌍 https://自分のURL.onrender.com<br/>スマホからも見られる！"]
```
...
```mermaid
flowchart LR
    Code["📁 blog-app<br/>（コード）"] --> D["📦 Dockerfile<br/>（箱の作り方）"]
    D --> Img["🧊 箱<br/>PHP・Laravel・CSS 入り"]
    Img --> Run["☁️ Render で動く"]
```
...
07 と同じです。`git push` すると自動で反映されます。
