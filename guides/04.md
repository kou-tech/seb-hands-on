...
```mermaid
flowchart TB
    subgraph goal["このページで作るもの"]
        Migration["📐 マイグレーション<br/>記事を保存する表の設計図"]
        Model["🗃 Article モデル<br/>記事1件を表すクラス"]
        Controller["🎛 ArticleController<br/>処理の司令塔"]
        Route["🗺 web.php<br/>URLの振り分け表"]
    end

    Migration -->|表を作る| DB[("🗄 database.sqlite")]
    Model <-->|記事を読み書き| DB
    Controller -->|記事をください| Model
    Route -->|URLごとに呼ぶ| Controller
```
...
たくさんのファイルがありますが、**今回さわるのは以下だけ**です。
...
**Laravel のウェルカムページ**が表示されれば成功です！🎉

まだ自分のブログではありませんが、**サーバーが動いている**証拠です。

### 2-3. これ以降の作業について

⚠️ **サーバーを起動したターミナルは、開いたままにしておきます。**

これ以降のコマンドは、**別のターミナル**を開いて実行してください。

```mermaid
flowchart LR
    T1["⌨️ ターミナル①<br/>php artisan serve<br/>（起動しっぱなし）"]
    T2["⌨️ ターミナル②<br/>ここで作業する"]
```

**別のターミナルの開き方**

- VS Code のメニュー「ターミナル」→「新しいターミナル」（`blog-app` の中で開くので便利です）
- または、ターミナルアプリで新しいウィンドウを開き、`cd ~/Desktop/blog-app` で移動

**サーバーを止めたいとき** … ターミナル①で `Ctrl + C` を押します。
...
**これが記事の保管庫です。** プロジェクト作成時に自動で作られているので、**何もしなくてOK**です。

```mermaid
flowchart LR
    App["🅻 Laravel"] -->|"記事を読み書き"| File["🗄 database/database.sqlite<br/>（ファイル1つ）"]
```
...
> 💡 **`-m` は「マイグレーションも一緒に作って」という意味**です。

> **メンター視点**
> - `Article` は**大文字始まり・単数形**です。Laravel はこの名前から自動で `articles` テーブルを探します
...
> - フォルダ内で **一番下（一番新しい）** のファイル、または名前に `articles` を含むファイルです
...
| id | title | published_on | body | created_at | updated_at |
|----|-------|--------------|------|-----------|-----------|
| 1 | 初めての記事 | 2026-09-05 | 今日は… | 自動 | 自動 |
| 2 | 札幌の秋 | 2026-09-12 | 大通公園で… | 自動 | 自動 |

| 書いたコード | 何ができる？ |
|------------|------------|
| `$table->id()` | 記事の番号（1, 2, 3…）。自動でつきます |
| `$table->string('title')` | **短い文字**を入れる列。タイトル用 |
| `$table->date('published_on')` | **日付**を入れる列。公開日用 |
| `$table->text('body')` | **長い文章**を入れる列。本文用 |
...
`app/Models/Article.php` を開いて、**中身をすべて消してから**以下を貼り付けます。
...
| 書いたコード | 意味 |
|------------|------|
| `#[Fillable([...])]` | **フォームから保存してよい項目**の指定。書いていない項目は保存されません（セキュリティ対策） |
| `casts()` の `'published_on' => 'date'` | 公開日を**日付として扱う**指定。`2026年9月5日` のような整形ができるようになります |
...
```mermaid
flowchart LR
    M["📐 マイグレーション<br/>（設計図）"] -->|"php artisan migrate"| T["🗄 articles テーブル<br/>（実際の表）"]
```
...
記事の**処理**（一覧・表示・作成・保存・編集・更新・削除）を担当する係を作ります。
...
```mermaid
flowchart LR
    User["👤 読む人"] -->|"① このURLが見たい"| Route["🗺 ルーティング<br/>web.php"]
    Route -->|"② 担当者を呼ぶ"| Controller["🎛 コントローラー<br/>ArticleController"]
    Controller <-->|"③ 記事をください"| Model["🗃 モデル<br/>Article"]
    Model <--> DB[("🗄 データベース")]
    Controller -->|"④ 記事を渡して画面を作る"| View["🖼 ビュー<br/>Bladeファイル"]
    View -->|"⑤ HTMLを返す"| User
```

| 役割 | 名前 | 担当 |
|------|------|------|
| **M**odel | モデル | **データ担当**。データベースとやりとりする |
| **V**iew | ビュー | **見た目担当**。HTMLを作る（05で作ります） |
| **C**ontroller | コントローラー | **司令塔**。M と V をつなぐ |
...
> 💡 **`--resource` とは？**
> 一覧・作成・保存・表示・編集・更新・削除という**よくある7つの処理**の枠を、
> 最初から用意してくれるオプションです。

### 5-2. コントローラーを編集

作られたファイルを開いて、**中身をすべて消してから**以下を貼り付けます。
...
| `store` | 「公開する」を押したとき | 入力内容を**保存**する |
| `edit` | 「編集」を押したとき | 編集フォームを表示 |
| `update` | 編集後に「更新」を押したとき | 内容を**書き換える** |
| `destroy` | 「削除」を押したとき | 記事を**消す** |
...
| `'required'` | 必須（空欄はダメ） |
| `'max:255'` | 255文字まで |
| `Article::create($validated)` | 記事を**新規保存** |
| `$article->update($validated)` | 記事を**書き換え** |
| `$article->delete()` | 記事を**削除** |
| `return view('articles.index', ...)` | `resources/views/articles/index.blade.php` を表示 |
| `redirect()->route('articles.index')` | 一覧ページに**移動させる** |
| `->with('success', '...')` | 「保存しました」のようなメッセージを1回だけ表示 |

> 💡 **`show(Article $article)` の不思議**
> URL に `/articles/3` と入ってくると、Laravel が**自動で3番の記事を探して**渡してくれます。
...
**どのURLで、どの処理を動かすか**を決めます。

`routes/web.php` を開いて、**中身をすべて消してから**以下を貼り付けます。
...
flowchart LR
    U1["/"] --> C1["index<br/>記事一覧"]
    U2["/articles/create"] --> C2["create<br/>新規作成画面"]
    U3["/articles/3"] --> C3["show<br/>3番の記事"]
    U4["/articles/3/edit"] --> C4["edit<br/>3番の編集画面"]
```
...
> | DELETE | 「消して」 |
>
> 同じ `/articles/3` でも、**GET なら表示、PUT なら更新、DELETE なら削除**になります。

> ⚠️ **書く順番が大事です**
> `/articles/create` は `/articles/{article}` **より上**に書いてください。
...
> ⚠️ ブラウザで `http://localhost:8000` を開くと、まだ**エラーが出ます**。
...
ここまでで、ブログの**基礎部分**（モデル・マイグレーション・コントローラー・ルーティング）が完成しました。

```mermaid
flowchart LR
    Done["✅ 04 で作った<br/>アプリの中身"] --> Next["👉 05 で作る<br/>アプリの見た目"]
```
