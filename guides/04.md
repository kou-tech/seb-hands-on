...
macOS と Windows はデスクトップ、Linux / WSL はホームの `projects` フォルダに作ります。
...
> 💡 ホーム（`~` と書きます）は、自分専用のフォルダです。
> Linux にはデスクトップにあたるフォルダがない環境もあるため、
> ホームの中に `projects` という作業用フォルダを作って、そこに置きます。

> ⚠️ WSL をお使いの方へ
> `/mnt/c/Users/...`（Windows 側のフォルダ）には置かないでください。
> ファイルの読み書きがとても遅くなり、`npm run build` が終わらなくなることがあります。
...
> ⚠️ Laravel 14 以降が出たあとに使うとき
> `laravel new` も `composer create-project laravel/laravel` も、そのときの最新版を取ってきます。
> このガイドは Laravel 13 で書かれているので、14 以降が出たあとは、以下のようにバージョンを指定して作ってください。
> ```bash
> composer create-project laravel/laravel:^13.0 blog-app
> ```
> 作ったあと `php artisan --version` を実行して、`Laravel Framework 13.x.x` と表示されれば大丈夫です。

> メンター視点
> - `laravel new` にはバージョンを指定するオプションがありません（Laravel Installer 5.x 時点）。13 に固定したいときは `composer create-project` を使ってください
> - 固定して作った場合、04 の質問（スターターキットやテストの選択）は出ません。Blade と PHPUnit の構成でそのまま進められます
...
> - WSL で `code` が使えない場合は、Windows 側の VS Code に `WSL` 拡張機能が入っているか確認してください（[03](./03.md) の③）
...
| 開き方 | すでに開いています | VS Code のメニュー「ターミナル」→「新しいターミナル」<br>または、ターミナルを新しく開いて `cd ~/Desktop/blog-app`（Linux / WSL は `cd ~/projects/blog-app`）で移動 |
| 止め方 | `Ctrl + C` | — |
